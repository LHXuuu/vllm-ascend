...
from vllm_ascend.ops.layernorm import AddRMSNormW8A8Quant, AddRMSNormW8A8DynamicQuant
...
class CustomQwen2DecoderLayer(Qwen2DecoderLayer):
    ...
    def __init__(
        ...
        if isinstance(self.self_attn.qkv_proj.quant_method.quant_method,
            ...
            self.input_layernorm = AddRMSNormW8A8DynamicQuant(
                config.hidden_size,
                layer=self.self_attn.qkv_proj,
                eps=config.rms_norm_eps)
        if isinstance(self.mlp.gate_up_proj.quant_method.quant_method,
                      AscendW8A8DynamicLinearMethod):
            self.post_attention_layernorm = AddRMSNormW8A8DynamicQuant(
                config.hidden_size,
                layer=self.mlp.gate_up_proj,
                eps=config.rms_norm_eps)
...
ALL_DECODER_LAYER_TYPES = {
    "attention": CustomQwen2DecoderLayer,
}
